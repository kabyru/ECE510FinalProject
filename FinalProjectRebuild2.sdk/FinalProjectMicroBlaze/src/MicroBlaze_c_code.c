...
int main (void) {
	...
	// Set the direction for all signals in channel 1 to be outputs (UPDATED 16 BITS)
	XGpio_SetDataDirection(&GpioDevice, 1, 0x0);
	// Set the direction for all signals in channel 2 to be inputs  (UPDATED 22 BITS)
	XGpio_SetDataDirection(&GpioDevice, 2, 0x3FFFFF);
	...
	while(1){
		...
		numA = DataRead & 0x3FF000;      //INPUT BITS [21:12] (COUNT_OUT)
		...
		if(DataRead != OldData){
            ...
            switch(op)
            {
                case 0 :    //ADD
                    ...
                    if(CompValue > 65535)
                    {
                        xil_printf("Result is greater than 16 bits. LED output may be incorrect due to bit width limitations.\r\n");
                        CompValue = 65535;
                    }
			...
			XGpio_DiscreteWrite(&GpioDevice, 1, CompValue);
